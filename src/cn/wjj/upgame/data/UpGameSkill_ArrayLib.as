package cn.wjj.upgame.data
{
	 ...
	 * UpGameSkill数组列表辅助类型
	 */
	public class UpGameSkill_ArrayLib
	{
		/** 对象引用的数据对象Array **/
		 ...
		 */
		public function UpGameSkill_ArrayLib(baseInfo:Array):void {
			__info = baseInfo;
			if(!ObjectPointer.hasArray(__info) && UpGameSkill.__keyList.length > 0)
			{
				ObjectPointer.createArrayKeyList(__info, UpGameSkill.__keyList);
			}
		}
		 ...
		 * @return
		 */
		public static function getThis(baseInfo:Array):UpGameSkill_ArrayLib
		{
			return new UpGameSkill_ArrayLib(baseInfo);
		}
		 ...
		 * @return
		 */
		public function getAll():Vector.<UpGameSkill> {
			var lib:Vector.<UpGameSkill> = new Vector.<UpGameSkill>();
			var item:Object;
			for each (item in __info){
				lib.push(new UpGameSkill(item));
			}
			return lib;
		}
		...
		public function getItemObj(p:String, vars:*, equal:String = "=="):Object{
			...
			if(equal == "==" && UpGameSkill.__keyList.indexOf(p) != -1)
			{
		 ...
		 * @return
		 */
		public function getItem(p:String, vars:*, equal:String = "=="):UpGameSkill{
			var item:Object = getItemObj(p, vars, equal);
			if (item) return new UpGameSkill(item);
			return null;
		}
		
		/**
		 * 从数组列表中查询子对象的属性名为p的值为vars,关系为equal的对象,返回Vector.<UpGameSkill>查到的对象,没有返回null
		 * @param	p	属性的名称
		 * @param	vars			值
		 * @param	equal			关系 : == != > < >= <= === !==
		 * @return
		 ...
		 * @return
		 */
		public function getList(p:String, vars:*, equal:String = "=="):Vector.<UpGameSkill> {
			var lib:Vector.<UpGameSkill> = new Vector.<UpGameSkill>();
		 ...
		 * @return
		 */
		private function __getInfo(progArr:Array, vars:Array, equal:Array, useAdd:Array):Vector.<UpGameSkill>
		{
			var lib:Vector.<UpGameSkill> = new Vector.<UpGameSkill>();
			if(__info == null)return lib;	
			var item:Object;
		 ...
		 * @return
		 */
		public function getItemArr(progArr:Array, vars:Array, equal:Array, useAdd:Array):UpGameSkill{
			var lib:Vector.<UpGameSkill> = __getInfo(progArr, vars, equal, useAdd);
			for each (var item:UpGameSkill in lib){
				return item;
			}
			return null;
		}
		 ...
		 * @return
		 */
		public function getListArr(progArr:Array, vars:Array, equal:Array, useAdd:Array):Vector.<UpGameSkill>{
