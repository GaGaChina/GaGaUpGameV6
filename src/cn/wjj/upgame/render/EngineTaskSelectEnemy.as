package cn.wjj.upgame.render 
{
	 ...
	 * @author GaGa
	 */
	public class EngineTaskSelectEnemy 
	{
		
		public function EngineTaskSelectEnemy() { }
		 ...
		 * @param	task
		 */
		public static function run(u:UpGame, task:TaskItemSelectEnemy, core:int):void
		{
			if (task.role)
			{
		...
		/** 通过ED数据来获取显示对象 **/
		private static function create(u:UpGame, task:TaskItemSelectEnemy, centerX:int, centerY:int):DisplayEDEffect
		{
			var display:DisplayEDEffect = DisplayEDEffect.instance();
			display.setDisplay(u, 0, "art/game/st_z_002.u2", centerX, centerY, u.reader.map.layer_flyEffectTop);
			u.reader.map.display_task[task] = display;
			return display;
		}
		
		/** 可以移除了 **/
		public static function remove(u:UpGame, task:TaskItemSelectEnemy):void
		{
			var display:DisplayEDEffect = u.reader.map.taskToDisplay(task) as DisplayEDEffect;
			delete u.reader.map.display_task[task];
			if (display)
